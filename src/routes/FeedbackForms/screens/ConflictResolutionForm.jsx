...
export const ConflictResolutionForm = () => {
  const navigate = useNavigate();
  const [questions, setQuestions] = useState([
    { id: 1, text: "How do I usually handle disagreements or misunderstandings?" },
    { id: 2, text: "Do I stay calm when challenged or criticized?" },
    { id: 3, text: "Am I open to feedback during conflict?" },
    { id: 4, text: "Do I avoid, confront, or resolve issues constructively?" },
  ]);
  ...
  const generateShareLink = () => {
    ...
    return `https://imirror.com/feedback/form?template=conflict-resolution&questions=${encodeURIComponent(JSON.stringify(questions))}`;
  };
  ...
  const handleShareWhatsApp = () => {
    const link = generateShareLink();
    const message = `Hey! I'd love your honest feedback on how I handle conflicts. Please fill out this quick form: ${link}`;
    ...
    <div className="bg-white flex flex-col items-center w-full min-h-screen">
      <div className="bg-[linear-gradient(136deg,rgba(219,234,254,1)_11%,rgba(202,225,254,1)_43%,rgba(252,231,243,1)_100%)] w-[381px] min-h-screen relative overflow-hidden pb-20">
        ...
        <div className="absolute w-full h-[120px] top-0 left-0 bg-white/80 backdrop-blur-sm shadow-sm flex flex-col items-center justify-center pt-10">
          ...
          <h1 className="font-bold text-inkdarkest text-xl mt-4">Conflict Resolution Feedback</h1>
          <p className="text-gray-700 text-sm text-center mt-1 px-4">
            Get perspective on how you manage disagreements and difficult conversations.
          </p>
        </div>
