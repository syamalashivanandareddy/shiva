...
export const CommunicationForm = () => {
  ...
  const [questions, setQuestions] = useState([
    { id: 1, text: "How clearly do I express my thoughts during conversations?" },
    { id: 2, text: "Do I interrupt others while they speak?" },
    { id: 3, text: "Do I listen actively and respond thoughtfully?" },
    { id: 4, text: "How do I handle disagreements during communication?" },
  ]);
  ...
  const generateShareLink = () => {
    ...
    return `https://imirror.com/feedback/form?template=communication&questions=${encodeURIComponent(JSON.stringify(questions))}`;
  };
  ...
  const handleShareWhatsApp = () => {
    const link = generateShareLink();
    const message = `Hey! I'd love your honest feedback on my communication skills. Please fill out this quick form: ${link}`;
    ...
    <div className="bg-white flex flex-col items-center w-full min-h-screen">
      <div className="bg-[linear-gradient(136deg,rgba(219,234,254,1)_11%,rgba(202,225,254,1)_43%,rgba(252,231,243,1)_100%)] w-[381px] min-h-screen relative overflow-hidden pb-20">
        ...
        <div className="absolute w-full h-[120px] top-0 left-0 bg-white/80 backdrop-blur-sm shadow-sm flex flex-col items-center justify-center pt-10">
          ...
          <h1 className="font-bold text-inkdarkest text-xl mt-4">Communication Feedback</h1>
          <p className="text-gray-700 text-sm text-center mt-1 px-4">
            Get insights into how you express yourself.
          </p>
        </div>
        ...
        <div className="w-[350px] mx-auto mt-[140px] pb-20 overflow-y-auto">
          {questions.map((q) => (
            <motion.div
              key={q.id}
              className="bg-white rounded-[15px] shadow-md p-4 mb-3 relative"
